...
# YOLO-FPGA Pipeline File Summary

This document explains the purpose of the files generated while running the YOLO → ONNX → Quantization → FPGA pipeline on the Raspberry Pi. It is intended to help contributors understand the data flow of the project.

---

# Pipeline Overview

The system converts a YOLO model into an ONNX format, extracts layer information, captures intermediate activations, performs quantization, and prepares the data required for FPGA execution.
...
Each stage generates files used by later steps.
...
A modified ONNX model where intermediate tensors (especially Conv layer inputs and outputs) are exposed as outputs. This allows ONNX Runtime to return internal activation values during inference.

Purpose:

* Used to capture intermediate tensor values
* Enables debugging and verification of each convolution layer
...
### yolo11n_pipeline_debug.onnx

A second debug version of the ONNX model used specifically for testing the FPGA pipeline implementation.

Purpose:

* Helps verify the FPGA pipeline produces correct results
* Used during pipeline simulation or hardware testing
...
## 2. Tensor Capture Files

### intermediate_tensors.npz

A compressed archive containing NumPy arrays representing the values of tensors flowing through the network.

Contents typically include:

* Conv layer input activations
* Conv layer output activations
* Preprocessed input image tensor

Purpose:

* Provides ground-truth values for validating FPGA computations
* Used during quantization to determine numeric ranges
...
Stores the floating-point convolution weights extracted from the ONNX model.
...
* Used as reference weights before quantization
...
### conv_weights_int8.npz

Quantized convolution weights converted from float32 to int8.

Purpose:

* Used by the FPGA convolution engine
...
Contains structural information about each convolution layer.
...
* Maps ONNX layers to FPGA pipeline stages
* Used by inference and FPGA simulation scripts
...
Stores quantization scaling factors for each convolution layer.
...
* Converts floating point activations to int8
* Ensures FPGA arithmetic produces correct results
...
### uart_comm.cpython-313.pyc

Compiled Python bytecode automatically generated by Python after running scripts.

Purpose:

* Speeds up future executions of Python modules
* Not required for the project to function

These files can safely be ignored or excluded from version control.

Recommended Git ignore entry:
...
A video recording was generated during testing of the camera pipeline. Typically contains a recorded camera stream or detection output.

Not required for FPGA processing.

---

### test_image.jpg

The test image used during inference to capture intermediate tensors.

Purpose:

* Provides consistent input when verifying FPGA results
...
| Script                | Purpose                                             |
| --------------------- | --------------------------------------------------- |
| export_onnx.py        | Converts YOLO model to ONNX format                  |
| inspect_graph.py      | Extracts convolution layer structure                |
| layer_inference.py    | Captures intermediate activation tensors            |
| quantize.py           | Quantizes weights and activations for FPGA          |
| fpga_pipeline.py      | Simulates or runs the FPGA inference pipeline       |
| uart_comm.py          | Handles communication between Raspberry Pi and FPGA |
| visionSystem.py       | Real-time vision pipeline                           |
| visionSystemStatic.py | Static image inference pipeline                     |
...
• `.npz` files store numerical tensors used during verification and quantization.
• `.json` files contain metadata describing the neural network structure.
• `.pyc` files are automatically generated and should not be committed.
• ONNX files represent the neural network computation graph.
